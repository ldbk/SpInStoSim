...
load("data/simu_geostat/SamplesLP_red.RData")
library(stringr)
...
na<-list(NA)
nstocks<-2 # change depending of the number of stocks (stocks*iteration*vario)
...
pb <- txtProgressBar(max = nstocks, style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)

#foreach(i = 1:2) %dopar%
tictoc::tic()
indices <- foreach(i=1:nstocks, .packages=c("VAST","FishStatsUtils"), .options.snow = opts) %dopar%{
  ...
  try(fit16$Report$Index_ctl[1,,1],TRUE)
}
tictoc::toc()
close(pb)
stopCluster()
